...
import com.barda.domain.datasource.service.impl.ClientBasedConnectionPool.ClientBasedDatasourceCacheKey;
import com.barda.impl.mock.MockConnection;
import com.barda.impl.mock.MockDatasourceConnectionConfig;
import com.barda.impl.mock.MockDatasourceConnector;
import com.barda.impl.mock.MockDatasourceMetaInfoService;
...
public class ClientBasedConnectionPoolTest {
    ...
    public void testConnectionExpired() {
        Datasource datasource1 = createDatasource(2);
        datasource1.setDetailConfig(new MockDatasourceConnectionConfig(datasource1));
        ClientBasedDatasourceConnectionHolder expired =
                (ClientBasedDatasourceConnectionHolder) POOL.getOrCreateConnection(datasource1).block();

        Datasource datasource2 = createDatasource(2);
        datasource2.setDetailConfig(new MockDatasourceConnectionConfig(datasource2));
        ClientBasedDatasourceConnectionHolder newConnection = (ClientBasedDatasourceConnectionHolder) POOL.getOrCreateConnection(datasource2).block();
        Assert.assertNotEquals(expired, newConnection);
        Assert.assertEquals(newConnection, getFromCacheDirectly(datasource2));
    }
