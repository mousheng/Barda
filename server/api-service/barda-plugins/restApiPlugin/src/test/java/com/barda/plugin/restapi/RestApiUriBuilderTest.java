...
public class RestApiUriBuilderTest {
    ...
    public void testPathConcatenation() {
        URI uri = RestApiUriBuilder.buildUri("http://cloud.barda.dev", "path?", Collections.emptyMap(), Collections.emptyMap());
        Assert.assertEquals("http://cloud.barda.dev/path?", uri.toString());
    }
    ...
    public void testExceptionCases() {
        ...
        Assert.assertThrows(PluginException.class, () -> {
            URI uri = RestApiUriBuilder.buildUri("8080:cloud.barda.dev", "", paramsMap, urlParams);
        });
    }
}
