...
This image contains all services needed to run Barda platform in one container.
...
```
DOCKER_BUILDKIT=1 docker build -f deploy/docker/Dockerfile -t bardadev/barda-ce .
```
...
| `API_SERVICE_ENABLED`           | If **true** barda api-service is started in the container      | `true`                                                  |
| `NODE_SERVICE_ENABLED`          | If **true** barda node-service is started in the container     | `true`                                                  |
| `FRONTEND_ENABLED`              | If **true** barda web frontend is started in the container     | `true`                                                  |
...
| `MONGODB_URI`                   | Mongo database connection string                                    | `mongodb://localhost:27017/barda?authSource=admin` |
...
| `ENCRYPTION_PASSWORD`           | Encryption password                                                 | `barda.dev`                                        |
| `ENCRYPTION_SALT`               | Salt used for encrypting password                                   | `barda.dev`                                        |
| `CORS_ALLOWED_DOMAINS`          | CORS allowed domains                                                | `*`                                                     |
| `BARDA_API_SERVICE_URL`    | Barda API service URL                                          | `http://localhost:8080`                                 |
| `BARDA_NODE_SERVICE_URL`   | Barda Node service (js executor) URL                           | `http://localhost:6060`                                 |


## Building api-service image

Standalone Barda api-service image.
...
Standalone Barda node-service (JS executor) image.
...
Standalone Barda web frontend image.
