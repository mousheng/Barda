...
本文档适用于初次使用 Barda 的新手。通过搭建一个简单的用户订单系统（如下图所示），快速了解 Barda 的主要组件及功能，熟悉 Barda 应用搭建流程。

![](assets/0-20231002133807-u1x7yxg.png)​

## 准备开始

注册与登录后，您将进入 Barda 的主页面，在这里创建和管理企业的应用。Barda 会为您预先创建一个示例应用，方便您快速体验 Barda 各项基础功能。

这里我们选择​**新建一个应用**​：

![](assets/2025-10-01-07-54-44.png)

## 使用组件

进入[编辑页面](editor-interface.md)后，使用组件来搭建应用。

1. 在页面右侧，Barda 为您提供了一系列功能丰富的组件。这里我们选择常用组件中的**文本**组件，并拖拽到页面中央的画布上。
...
1. 在页面下方的查询编辑器中，可以编写 MySQL、PostgreSQL、MongoDB、REST API 等多种数据源的查询语句。Barda 为您预先创建了​**示例数据源**​，并填充了一些演示数据；此外，您可以[新建数据源](datasource.md)并基于其创建查询，也可以在编辑器中的**选择数据源**下拉菜单中切换数据源。
2. 使用 MySQL 示例数据源**新建**一个查询，并将其重命名为 `getUsers`​：
...
2. 为了展示上述查询结果，需要将表格的**数据**属性与 `{{getUsers.data}}`​ 进行绑定。这里表格自动绑定了刚刚执行的 `getUsers`​ 的结果，后续当 `{{getUsers.data}}`​ 发生变化时，表格中展示的内容也会同步更新。

![](assets/5-20231002133807-62hnbs3.png)​

```sql
-- Barda 使用 {{ }} 语法标记 JavaScript 表达式并自动计算对应的值，比如对于下方的 SQL：
select * from products 
where price_in_fen > {{ input1.value * 100 }} and price_in_fen < {{ input2.value * 100 }}
-- 当 input1.value 为 10，input2.value 为 50 时，Barda 编辑器会分别解析出两个变量 {{ input1.value * 100 }} 与 {{ input2.value * 100 }} ，并求出对应的值：1000 与 5000。
```
...
1. 当表格数据较多时，搜索功能可以更快定位数据。这里我们插入一个**输入框**组件 `input1`​，拖拽到表格上方，并修改**标签文本**为「姓名」。
...
-- Barda 默认针对 {{ }} 中的值开启预编译语句 (PreparedStatement)，因此写成 {{'%' + input1.value + '%'}}
-- 预编译语句模式下不能写成 '%{{ input1.value}}%'
```

![](assets/8-20231002133807-wgb6htj.png)​

3. 改写后的 SQL 语句引用了 `input1.value`​，即 `input1`​ 的值，并结合 MySQL 通配符实现了按照输入的关键字进行模糊搜索的功能。这里我们试着搜索「子」，可以看到对应的查询结果。

![](assets/9-20231002133807-4w0sqap.png)​

## 获取用户订单

使用表格组件的属性构建新的查询，获取选中用户的所有订单。
...
2. 表格组件的 **selectedRow** 属性表示当前选中的行。`getUserOrders`​ 能从表 orders 中获取 `table1`​ 当前选中行所对应的用户的所有订单。这里我们插入一个表格组件 `table2`​，并将数据字段与 `{{getUserOrders.data}}`​ 绑定。此时，选中 `table1`​ 中的任意一行，`table2`​ 将随之更新，展示所选用户的所有订单。

![](assets/11-20231002133807-l68rul2.png)​

3. 为了明确展示是谁的订单，这里我们可以插入一个​**容器**​，将 `table2`​ 拖入容器内，并将容器标题的内容修改为 `### {{table1.selectedRow.name}}的订单`​。此时，若选中 `table1`​ 中「龚子默」这一行，容器的标题就会随之更新为「龚子默的订单」。
...
1. 同样为了提高可读性，修改 **status** 列的单元格值为 `{{currentCell == 'finished' ? '完成' : '已下单'}}`​，将数据映射为文本，再把列类型设为标签。可以看到，表格中某些订单的状态是​**已下单**​，需要增加更新订单状态为**完成**的功能。这里我们插入一个**按钮**组件 `button1`​，拖拽到表格组件 `table2`​ 上方，并修改**文本**为「标记为完成」。
...
3. 更新语句 `updateOrderStatus`​ 执行成功后，当前用户的订单表也要随之更新，所以还需要绑定成功后的回调事件。这里我们为 `updateOrderStatus`​ 新建一个[事件触发](event-handler.md)，选择成功后执行查询 `getUserOrders`​。
...
好了，新手旅程告一段落。Barda 还为您提供了一系列开箱即用的组件并支持多种数据源连接，欢迎您进一步探索与体验！
