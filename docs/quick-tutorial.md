...
本文档适用于初次使用Barda的新手。通过搭建一个简单的用户订单系统（如下图所示），快速了解Barda的主要组件及功能，熟悉Barda应用搭建流程。

![](assets/0-20231002133807-u1x7yxg.png)​

## 准备开始

注册与登录后，您将进入Barda的主页面，在这里创建和管理企业的应用。Barda会为您预先创建一个示例应用，方便您快速体验Barda各项基础功能。
...
1. 在页面右侧，Barda为您提供了一系列功能丰富的组件。这里选择常用组件中的**文本**组件，并拖拽到页面中央的画布上。
...
1. 在页面下方的查询编辑器中，可以编写 MySQL、PostgreSQL、MongoDB、REST API 等多种数据源的查询语句。Barda为您预先创建了​**示例数据源**​，并填充了一些演示数据；此外，您可以[新建数据源](datasource.md)并基于其创建查询，也可以在编辑器中的**选择数据源**下拉菜单中切换数据源。
...
```sql
-- Barda使用 {{ }} 语法标记 JavaScript 表达式并自动计算对应的值，比如对于下方的 SQL：
select * from products 
where price_in_fen > {{ input1.value * 100 }} and price_in_fen < {{ input2.value * 100 }}
-- 当 input1.value 为 10，input2.value 为 50 时，Barda编辑器会分别解析出两个变量 {{ input1.value * 100 }} 与 {{ input2.value * 100 }} ，并求出对应的值：1000 与 5000。
```
...
-- Barda默认针对 {{ }} 中的值开启预编译语句 (PreparedStatement)，因此写成 {{'%' + input1.value + '%'}}
...
好了，新手旅程告一段落。Barda还为您提供了一系列开箱即用的组件并支持多种数据源连接，欢迎您进一步探索与体验！
