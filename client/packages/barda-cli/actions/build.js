...
function validPackageJSON() {
  ...
  if (!packageJSON.barda) {
    return "- barda field is required in package.json";
  }
  const barda = packageJSON.barda;
  if (!barda.comps || Object.keys(barda.comps).length === 0) {
    return "- not found any comps to build";
  }

  const compErrors = [];
  Object.keys(barda.comps).forEach((name) => {
    const compManifest = packageJSON.barda.comps[name];
...
export default async function buildAction(options) {
  ...
  const compNames = Object.keys(packageJSON.barda.comps);
  ...
  // write package.json
  packageJSON.barda.entry = "index.js";
