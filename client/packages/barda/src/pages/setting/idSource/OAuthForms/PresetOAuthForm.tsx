...
import { messageInstance, CloseEyeIcon } from "barda-design";
...
type PresetOAuthFormProp = {
  authType: AuthType,
  onSave: () => void;
  onCancel: () => void;
};

function PresetOAuthForm(props: PresetOAuthFormProp) {
  const {
    authType,
    onSave,
    onCancel,
  ...
  function saveAuthProvider(values: ConfigItem) {
    setSaveLoading(true);
    const { enableRegister, ...restValues } = values;
    const config = {
      ...restValues,
      authType,
...
export default PresetOAuthForm;
