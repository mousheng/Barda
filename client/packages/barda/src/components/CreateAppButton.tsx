...
// 定义选择器以进行记忆化
const makeSelectSelectedState = () =>
  createSelector([
    (state: AppState) => state.ui.users.user,
    (state: AppState) => state.ui.application.loadingStatus.isApplicationCreating
...
export function useCreateApp(type: AppTypeEnum, onSuccess?: (app: ApplicationDetail) => void) {
  ...
  const selectSelectedState = useCallback(makeSelectSelectedState(), []);
  const { currentUser, isCreating } = useSelector(selectSelectedState);
