...
import { CheckBox, controlItem, Switch, SwitchJsIcon, SwitchWrapper } from "barda-design";
      ...
      <SwitchWrapper {...params}>
        {params.type === "checkbox" ? (
          <CheckBox
            ...
            checked={this.value}
            onChange={(x: CheckboxChangeEvent) => this.dispatchChangeValueAction(x.target.checked)}
          />
        ) : (
...
const CodeEditorWrapper = styled.div<{ $hasLabel: boolean, $useCodeEditor: boolean }>`
  flex: ${(props) => (!props.$hasLabel ? "1" : props.$useCodeEditor ? "1" : "")}
`;
