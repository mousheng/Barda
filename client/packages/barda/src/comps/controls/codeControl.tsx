...
import { hourStepType, minuteStepType, secondStepType } from "../comps/dateComp/dateComp";
...
export const ColorCodeControl = codeControl<string>(
  (value: unknown) => {
    ...
    throw new Error(trans("style.inputColorError"));
  },
  {
...
export const ColorOrBoolCodeControl = codeControl<string>(
  (value: unknown) => {
    ...
    throw new Error(trans("style.inputColorOrBooleanError"));
  },
  {
...
function isValidCSS(propertyName: string, borderValue: string) {
  return CSS.supports(propertyName, borderValue);
}

export const BorderControl = codeControl<string>(
  (value: unknown) => {
    const valueString = toString(value);
    if (isValidCSS("border", valueString) || valueString === "") {
      return valueString;
    }
    throw new Error(trans("style.inputBorderError"));
  },
  {
    expectedType: "CSS",
  }
...
export const CssUnitControl = codeControl<string>(
  (value: unknown) => {
    const valueString = toString(value);
    if (isValidCSS("padding", valueString) || valueString === "") {
      return valueString;
    }

    // 支持 1-4 个以空格分隔的数字，不带单位时默认使用 px
    const trimmedValue = valueString.trim();
    if (trimmedValue === "") {
      return valueString;
    }

    const numberPattern = /^(\d+(?:\.\d+)?)(?:\s+(\d+(?:\.\d+)?))?(?:\s+(\d+(?:\.\d+)?))?(?:\s+(\d+(?:\.\d+)?))?\s*$/;
    const match = trimmedValue.match(numberPattern);
    ...
    if (match) {
      ...
      throw new Error(trans("style.inputPaddingOrMarginError"));
  },
  {
    expectedType: "CSS",
  }
...
export const RadiusControl = codeControl<string>(
  (value: unknown) => {
    const valueString = toString(value);
    if (valueString === "" || isValidCSS("border-radius", valueString)) {
      return valueString;
    }
    if (/^[0-9]+\s*$/.test(valueString)) {
      return valueString.trim() + "px";
    }
    throw new Error(trans("style.inputRadiusError"));
  },
  {
    expectedType: "CSS",
  }
