...
import { CustomSelect, EllipsisTextCss } from "barda-design";
...
const BardaAPIValue: ResourceOptionValue = {
  id: BARDA_API_ID,
  type: "bardaApi",
};
    ...
    <SelectWrapper>
      ...
      >
        ...
        {context?.placement !== "queryLibrary" && (
          <>
            <SelectOption
              key={JSON.stringify(BardaAPIValue)}
              label={BARDA_API_INFO.name}
              value={JSON.stringify(BardaAPIValue)}
            >
              <SelectOptionContains>
                {BARDA_API_INFO.icon}
                <SelectOptionLabel>{BARDA_API_INFO.name} </SelectOptionLabel>
              </SelectOptionContains>
            </SelectOption>
