import {
  ...
  QueryMap,
  ResourceType,
} from "@barda/constants/queryConstants";
...
import { ValueFromOption } from "barda-design";
...
const checkoutCodeWithoutReservedWords = (code: String): boolean => {
  if (isEmpty(code) || code.length > 100000) return false;
  const reservedWords = [
    "window",
    "globalThis",
    "self",
  ...
  return !some(reservedWords, (reservedWord) => code.includes(reservedWord));
}
...
QueryCompTmp = class extends QueryCompTmp {
  ...
  changeCommentary(value: string) {
    this.children.commentary.dispatch(this.children.commentary.changeValueAction(value))
  }
    ...
    this.children.confirmationModal
      .getView()(() => {
        ...
        return queryFunc({
          ...
          withOutReservedWords: checkoutCodeWithoutReservedWords((this.children.comp.children as any)?.script?.unevaledValue),
        });
