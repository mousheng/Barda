...
import { CompAction, customAction, isMyCustomAction } from "barda-core";
import { message } from "antd";

const safeMessageInstance = messageInstance ? messageInstance : message;

const SuccessMessageAction = new MultiCompBuilder(
  {
    text: StringControl,
  },
  (props) => (duration: number) => props.text && safeMessageInstance.success(props.text, duration)
)
...
const QueryNotificationTmpControl = new MultiCompBuilder(
  ...
  (props) => (name: string, triggerType: TriggerType, result: QueryResult) => {
    ...
    if (props.showFail) {
      props.fail.forEach((item) => {
        ...
        if (props.condition && props.text) {
          safeMessageInstance.error(props.text, duration);
          hasNoticed = true;
        }
      });
      ...
      if (!result.success && !hasNoticed) {
        hasNoticed = !!safeMessageInstance.error(
    ...
    if (!hasNoticed && props.showSuccess && triggerType === "manual") {
      ...
      if (result.success && !hasNoticed) {
        safeMessageInstance.success(trans("query.successMessageWithName", { name }), duration);
      }
    }
  }
)
