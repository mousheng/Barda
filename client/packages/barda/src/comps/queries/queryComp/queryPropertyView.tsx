import { OLD_BARDA_DATASOURCE } from "@barda/constants/datasourceConstants";
import { manualTriggerResource, ResourceType } from "@barda/constants/queryConstants";
...
import { deferAction, executeQueryAction, wrapActionExtraInfo } from "barda-core";
import {
  CustomModal,
  Dropdown,
...
export function QueryPropertyView(props: { comp: InstanceType<typeof QueryComp> }) {
  ...
  const commentary = children.commentary.getView();
  return (
    <BottomTabs
...
export const QueryGeneralPropertyView = (props: {
  ...
  // transfer old Barda API datasource to new
  const oldBardaId = useMemo(
    () =>
      datasource.find(
        (d) =>
          d.datasource.creationSource === 2 && OLD_BARDA_DATASOURCE.includes(d.datasource.type)
      )?.datasource.id,
    [datasource]
  );
  if (datasourceId === oldBardaId) {
    datasourceId = BARDA_API_ID;
    datasourceType = "bardaApi";
    dispatch(
