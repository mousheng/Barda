...
import { DateParser, TimeParser, fromStringSafeGetDayjs } from "util/dateTimeUtils";
...
export const disabledDate = (current: dayjs.Dayjs, min: dayjs.Dayjs | undefined, max: dayjs.Dayjs | undefined) => {
  // const maxDate = fromStringSafeGetDayjs(max, DateParser);
  // const minDate = fromStringSafeGetDayjs(min, DateParser);
  return (
    current &&
    current.isValid() &&
    (current.isAfter(max, "date") || current.isBefore(min, "date"))
  );
};

export const disabledTime = (min: string, max: string) => {
  const maxTime = fromStringSafeGetDayjs(max, TimeParser);
  const minTime = fromStringSafeGetDayjs(min, TimeParser);
  return {
    disabledHours: () => {
...
export const dateRefMethods = refMethods<HTMLElement>([focusMethod, blurMethod]);
