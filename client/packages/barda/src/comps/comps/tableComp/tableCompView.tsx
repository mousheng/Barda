...
import { darkenColor, isDarkColor, messageInstance } from "barda-design";
...
import { RowColorViewType, TableEventOptionValues } from "comps/comps/tableComp/tableTypes";
import {
  COL_MIN_WIDTH,
  COLUMN_CHILDREN_KEY,
  columnsToAntdFormat,
 ...
 */
const ResizeableTable = memo(<RecordType extends object>(props: CustomTableProps<RecordType>) => {
  const [resizeData, setResizeData] = useState({
    index: -1,
    width: -1,
  });
  let allColumnFixed = true;
  const columns = useMemo(() => props.columns.map((col, index) => {
  ...
  }), [props.columns, props.viewModeResizable, props.rowColor, resizeData]);
  ...
  const expandable = useMemo(() => props.expandable, [props.expandable])
  const scroll = useMemo(() => ({ x: COL_MIN_WIDTH * columns.length }), [columns.length])
  const rowClassName = useMemo(() => props.rowClassName, [props.rowClassName])
...
export function TableCompView(props: {
  ...
  const styleData = compChildren.style.getView();
  const style = useMemo(() => styleData, [compChildren.style]);
  ...
  const expandable = useMemo(() => ({
    ...
    onExpand: (expanded: boolean, record: RecordType) => {
      compChildren.selection.dispatch(compChildren.selection.changeChildAction("selectedRowKey", record[OB_ROW_ORI_INDEX]));
    }
  }), [compChildren.selection, expansion.expandableConfig, supportChildren])

  const rowColor = useMemo(() => compChildren.rowColor.getView(), [compChildren.rowColor])

  const change = useCallback((pagination: any, filters: any, sorter: any, extra: any) => {
    onTableChange(pagination, filters, sorter, extra, comp.dispatch, onEvent);
  }, [comp.dispatch, onEvent])
  ...
  const toolbarView = (
    <TableToolbar
      ...
      onCancelChanges={onCancelChangesCallback}
      onEvent={onEvent}
    />
  );
    ...
    <BackgroundColorContext.Provider value={style.background}>
      <TableWrapper ref={ref} $style={style} $toolbarPosition={toolbar.position}>
        ...
        <ResizeableTable
          ...
          rowColor={rowColor}
          onRow={selectionEvents.onRow as any}
          rowClassName={selectionEvents.rowClassName as any}
          rowKey={selectionEvents.rowKey as GetRowKey<object>}
          rowSelection={selectionEvents?.rowSelection as any}
          bordered={!compChildren.hideBordered.getView()}
          onChange={change}
          showHeader={!compChildren.hideHeader.getView()}
          columns={antdColumns as CustomColumnType<object>[]}
