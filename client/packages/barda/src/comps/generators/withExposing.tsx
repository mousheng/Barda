...
import { fromRecord, fromValue, Node, withFunction } from "barda-core";
...
import { CompAction, customAction, isMyCustomAction, ExtraNodeType } from "barda-core";
...
/**
 * 默认情况下，具有 hidden 和 disabled 属性的组件会暴露这两个属性
 */
...
/**
 * 构造一个复杂的配置对象，用于公开数据，如果需求简单，建议使用 NameConfig
 * @param name 公开数据的名称
 * @param depNodesFn 返回依赖节点的函数，注意需要返回一个缓存版本（当前节点函数已有缓存）
 * @param func 通过依赖节点计算新值的函数
 * @param desc 描述性文档，可选
 */
constructor(
  name: string,
  ...
  this.name = name; // 公开数据的名称
  this.desc = desc ?? ""; // 描述性文档，默认为空字符串
  this.depNodesFn = depNodesFn; // 返回依赖节点的函数
  this.func = func; // 计算新值的函数
}
  ...
  implements ExposingConfig<Comp>
{
  ...
  /**
   * 一个复杂的公开数据配置，简单情况下可以使用 NameConfig
   * @param name 公开的名称
   * @param depNodesFn 返回依赖节点，注意需要一个缓存版本（当前节点函数已有缓存）
   * @param func 通过依赖节点计算新值
   * @param desc 描述性文档
   */
  constructor(
    name: string,
