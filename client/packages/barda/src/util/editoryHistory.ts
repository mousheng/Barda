import { Comp, CompAction, CompActionTypes, isBroadcastAction, RenameAction } from "barda-core";
...
export function useAppHistory(compContainer: CompContainer, readOnly: boolean, appId: string) {
  ...
  return useMemo(() => {
    ...
    compContainer.addChangeListener((actions) => {
      ...
      // 可能较慢：通过 `toJson` 比较 DSL
      // 节流可能在连续派遣两个动作时导致动作丢失（例如添加表格、删除）
      // 尝试在 1.5 毫秒内处理 100 个组件，如果进一步变慢则解决此问题
