...
const ChildWrapper = styled.div<{ $isDragging: boolean }>`
  height: 100%;
 ...
 */
export function GridItemTemmp(props: GridItemProps) {
  const position = calcGridItemPosition(props, props.x, props.y, props.w, props.h);
  const handleFlag = useRef<"onResizeStart" | "onResize" | "onResizeStop">("onResizeStop");
  ...
  const mixinResizable = (
    ...
    const mins = calcGridItemPosition(props, 0, 0, minW as number ?? 1, minH as number ?? 1);
  ...
  const onResizeStop = (e: React.SyntheticEvent, callbackData: ResizeCallbackData) => {
    handleFlag.current = "onResizeStop";
  ...
  const onResizeStart = (e: React.SyntheticEvent, callbackData: ResizeCallbackData) => {
    handleFlag.current = "onResizeStart";
    onResizeHandler(e, callbackData);
  };
  ...
  const onResizeHandler = (
    ...
    e.preventDefault();
    e.stopPropagation();
    const handler = props[handleFlag.current];
    if (!handler) return;
    const oldSize = resizing;
    const newSize = size;
    newSize.width = Math.round(Math.round(newSize.width / colWidth) * colWidth);
    ...
    if (resizeNorth || resizeWest) {
      if (handleFlag.current === "onResizeStart") {
        localDragging = {
          left: position.left,
          top: position.top,
        };
        ...
        if (handleFlag.current === "onResize") {
          let deltaX = 0;
          let deltaY = 0;
          if (oldSize) {
    ...
    setResizing(handleFlag.current === "onResizeStop" ? undefined : newSize);
    setDragging(handleFlag.current === "onResizeStop" ? undefined : localDragging);

    // Get new XY
    let { w: ww, h: hh } = calcWH(props, newSize.width, newSize.height);
    // Min/max capping
    ww = clamp(ww, minW ?? 1, maxW ?? Infinity);
    hh = clamp(hh, minH ?? 1, maxH ?? Infinity);
  ...
  const adjustWrapperHeight = (width?: number, height?: number, src?: string) => {
    ...
    const padding = props.compType === "module" ? [0, 0, 0, 0] : props.itemMargin ?? [2.5, 6.5, 2.5, 6.5];
    const { h } = calcWH(props, width + padding[1] + padding[3], height + padding[0] + padding[2], {
      w: true,
      h: true,
      ceil: true,
    });
  ...
  /**
 * 重新计算被占用的网格单元格。
 * 当 isRowCountLocked 为 true 时，包裹器的高度会发生变化，需要重新计算。
 *
 * 在项的包裹器尺寸发生变化且 autoHeight 为 true 时被调用。
 */
  ...
  const mixinChildWrapper = (child: React.ReactElement): React.ReactElement => {
    ...
    return (
      <CompSelectionWrapper
        ...
        resizeHandles={resizeHandles}
      >
        <ChildWrapper $isDragging={isDragging! && !!props.isSelected && props.i !== "moduleContainer"}>
          {child}
        </ChildWrapper>
      </CompSelectionWrapper>
    );
  ...
  const calcPosition = useCallback((): Position => {
    ...
    if (resizing) {
      ...
      height = Math.round(handleFlag.current === "onResize" && props.autoHeight ? itemHeightRef.current! : resizing.height);
      if (handleFlag.current === "onResize" && props.autoHeight) {
        height = Math.ceil((itemHeightRef.current! + (props.itemMargin?.[0] ?? 2.5) + (props.itemMargin?.[2] ?? 2.5)) / 8) * 8
      } else {
