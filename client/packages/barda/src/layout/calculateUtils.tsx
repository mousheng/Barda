...
import { getPanelStatus, getPanelStyle } from "@barda/util/localStorageUtil";
import { TopHeaderHeight } from "@barda/constants/style";
...
/**
 * 给定像素值的高度和宽度，计算网格单位。
 * @param  {PositionParams} positionParams  计算坐标所需的网格参数。
 * @param  {Number} height                  以像素为单位的高度。
 * @param  {Number} width                   以像素为单位的宽度。
 * @param  {Number} x                       以网格单位为单位的 X 坐标。
 * @param  {Number} y                       以网格单位为单位的 Y 坐标。
 * @return {Object}                         返回以网格单位为单位的 w 和 h。
 */
export function calcWH(
  ...
  itemMargin: [number, number, number, number] = [2.5, 6.5, 2.5, 6.5]
): { w: number; h: number } {
...
export function genPositionParams(props: GridLayoutProps): PositionParams {
  const { cols, margin, maxRows, rowHeight, gridWidth, containerPadding } =
  ...
  const params = {
    ...
    colWidth: (gridWidth - margin[0] * (cols - 1) - containerPadding[0] * 2) / cols,
  };
  return params;
}
...
// 根据元素的位置，判断元素是否在可视区域内并返回滚动位置
export function getElementScrollPosition(rect: DOMRect): ScrollLogicalPosition | "none" {
  if (!rect) return "nearest";
  const headerHeight = parseInt(TopHeaderHeight);
  const canvasHeight = document.documentElement.clientHeight - getPanelStyle().bottom.h * (getPanelStatus().bottom ? 1 : 0);
  const isLargeComponent = rect.height >= canvasHeight;
  // 完全超出画布
  // 在上方完全不可见：不滚动
  if (rect.bottom <= headerHeight) return "nearest";
  // 在下方完全不可见
  if (rect.top >= canvasHeight) return isLargeComponent ? "start" : "end";
  ...
  // 被底部部分遮挡
  if (rect.top < canvasHeight && rect.bottom > canvasHeight)
    return isLargeComponent ? "start" : "end"
