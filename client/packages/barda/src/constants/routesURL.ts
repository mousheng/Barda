...
export const BRANDING_SETTING = "/setting/branding";
...
export function preview(applicationId: string, previewTab?: WindowProxy | null) {
  const url = APPLICATION_VIEW_URL(applicationId, "preview");
  if (previewTab && previewTab?.closed !== true) {
