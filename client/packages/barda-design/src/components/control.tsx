...
const LabelWrapper = styled.div<{
  ...
  $autoHideLastNode?: boolean;

}>`
  ${(props) => {
    switch (props.$layout) {
      case "horizontal":
        return props.$autoHideLastNode ? css`
          width: 78px;
        ` : css`
...
const LastNode = styled.div<{ $autoHideLastNode: boolean }>`
  display: inline-flex;
  align-items: center;
  opacity: ${(props) => (props.$autoHideLastNode ? 0 : 1)};
...
export const ControlPropertyViewWrapper = (
  ...
  const {
    ...
    lastNode,
    labelStyle,
    autoHideLastNode = false,
  } = props;
  const styles = useMemo(() => ({ body: { ...labelTooltipOverlayInnerStyle } }), [labelTooltipOverlayInnerStyle]);
  const LastNodeComp = useMemo(() => lastNode && <LastNode className="last-node" $autoHideLastNode={autoHideLastNode}>{lastNode}</LastNode>, [lastNode, autoHideLastNode])
    ...
    <Wrapper $layout={layout} $placement={placement}>
      {label && (
        <LabelWrapper
          ...
          style={labelStyle}
          $autoHideLastNode={!!LastNodeComp && autoHideLastNode}
        >
          <ToolTipLabel
            title={tooltip}
            label={label}
