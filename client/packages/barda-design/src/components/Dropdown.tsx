...
const SegmentedWrapper = styled.div<{ $placement: ControlPlacement }>`
  display: flex;
  height: 28px;
  width: ${(props) => (props.$placement === "right" ? "calc(100% - 98px)" : "100% - 112px")};
  flex-grow: 1;
...
const LabelWrapper = styled.div<{ $placement: ControlPlacement }>`
  flex-shrink: 0;
  width: ${(props) => (props.$placement === "right" ? "98px" : props.$placement === "bottom" ? "112px" : "136px")};
`;
    ...
    <FlexDiv style={props.style} className={props.className}>
      {props.label && (
        <LabelWrapper $placement={placement} style={props.labelStyle}>
          <ToolTipLabel title={props.toolTip} label={props.label} labelStyle={props.labelStyle} />
        </LabelWrapper>
      )}
      ...
      {props.radioButton && (
        <SegmentedWrapper $placement={placement}>
          <Segmented
            block={true}
            onChange={(value: unknown) => props.onChange((value as string).toString())}
            defaultValue={props.defaultValue}
            value={props.value}
