...
function Tooltip(props: TooltipProps) {
  const styles = useMemo(() => ({ body: { ...overlayInnerCss } }), [overlayInnerCss]);
  return <AntdTooltip color="#2c2c2c2" styles={styles} {...props} />;
}

const Label = styled.div<{ $border?: boolean, $labelColor?: string }>`
  span {
...
function ToolTipLabel(
  ...
  const styles = useRef({ body: { maxWidth: "232px", whiteSpace: "break-spaces" } });
  return (
    <AntdTooltip
      color="#2c2c2c"
    ...
    >
      {label ? (
        <Label style={labelStyle} $border={!!title} className="tooltipLabel" $labelColor={props.labelStyle?.color}>
          <span>{label}</span>
        </Label>
      ) : (
        children
