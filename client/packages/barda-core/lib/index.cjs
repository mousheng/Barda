...
var reallyRelaxedJson = require('really-relaxed-json');
...
function relaxedJSONToJSON(text, compact) {
    ...
    return reallyRelaxedJson.toJson(text, compact);
}
...
function isCustomAction(action, type) {
    return action.type === exports.CompActionTypes.CUSTOM && _.get(action.value, "type") === type;
}
/**
 * 执行查询的动作。
 * 精确地指向查询的路径路由。
...
function isBroadcastAction(action, type) {
    return action.type === exports.CompActionTypes.BROADCAST && _.get(action.action, "type") === type;
}
...
var Translator = /** @class */ (function () {
    ...
    Translator.prototype.transToNode = function (key, variables) {
        ...
        var node = new intlMessageformat.IntlMessageFormat(message, i18n.locale).format(variables);
        if (Array.isArray(node)) {
            return node.map(function (n, i) { return jsxRuntimeExports.jsx(require$$0.Fragment, { children: n }, i); });
        }
        return node;
    };
...
exports.promiseWithDefaultOnTimeout = promiseWithDefaultOnTimeout;
