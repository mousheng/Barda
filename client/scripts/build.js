...
import { currentDirName, readJson } from "barda-dev-utils/util.js";

const builtinPlugins = ["barda-comps"];
...
async function downloadBuiltinPlugin(name) {
  ...
  const targetDir = `./packages/barda/build/${name}/latest`;
...
shell.exec(`BUILD_TARGET=browserCheck yarn workspace barda build`, { fatal: true });
shell.exec(`yarn workspace barda build`, { fatal: true });
...
if (process.argv.includes("--internal-deploy")) {
  ...
  shell.exec("docker cp ./packages/barda/build barda-fe:/var/www/", { fatal: true });
  shell.exec(
    `docker exec barda-fe /bin/sh -c "cd /var/www/ && rm -rf ${deployDir} && mv build ${deployDir}"`,
    { fatal: true }
  );
}
